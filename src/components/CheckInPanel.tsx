...
import { Camera, Loader2, CheckCircle, XCircle, ScanLine, Map, UtensilsCrossed, ListOrdered } from 'lucide-react'
...
export function CheckInPanel() {
  ...
  const foodMutation = useMutation({
    ...
    onSuccess: (data) => {
      const name = data.user_name
      const table = data.table?.label ? `โต๊ะ ${data.table.label}` : `#${data.table_id}`
      toast.success('ยืนยันการรับอาหารแล้ว', { description: `${name} · ${table}` })
      queryClient.invalidateQueries({ queryKey: ['tables'] })
      queryClient.invalidateQueries({ queryKey: ['bookings'] })
      setSelectedTable(null)
      setModalMapMode(null)
  ...
  useEffect(() => {
    ...
    import('html5-qrcode').then(({ Html5Qrcode }) => {
      if (!mounted) return
      if (!document.getElementById('qr-reader-admin')) return
      ...
      <Tabs value={tab} onValueChange={setTab} className="w-full">
        <TabsList className="grid w-full grid-cols-2 sm:grid-cols-4 mb-4">
          ...
          <TabsTrigger value="map-attendance" className="gap-1 sm:gap-2">
            ...
            <span className="hidden sm:inline">เข้าร่วมงาน</span>
          </TabsTrigger>
          <TabsTrigger value="map-food" className="gap-1 sm:gap-2">
            <UtensilsCrossed className="w-4 h-4 shrink-0" />
            <span className="hidden sm:inline">รับอาหาร</span>
          </TabsTrigger>
          <TabsTrigger value="queue-food" className="gap-1 sm:gap-2">
            <ListOrdered className="w-4 h-4 shrink-0" />
            <span className="hidden sm:inline">คิวอาหาร</span>
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="scan" className="mt-0">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                ใช้กล้องสแกน QR บนตั๋ว หรือพิมพ์รหัสการจอง
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <form onSubmit={handleManualSubmit} className="space-y-3">
                <div className="space-y-2">
                  <Label htmlFor="booking-id-admin">รหัสการจอง (Booking ID)</Label>
                  <Input
                    id="booking-id-admin"
                    type="text"
                    value={bookingId}
              ...
              {cameraOpen && (
                <div id="qr-reader-admin" className="rounded-lg overflow-hidden border border-gray-200" />
              )}
              {lastResult && (
                <div
        ...
        <TabsContent value="map-attendance" className="mt-0">
          <div className="rounded-xl bg-white shadow-md p-2 sm:p-4 mb-4">
          ...
          <div className="w-full max-w-[1100px] mx-auto aspect-[13/9] min-h-[280px] overflow-auto rounded-xl border border-gray-200 bg-white shadow-inner">
            <CheckInTableGrid
              fitViewport
              onTableSelect={(t) => {
                setSelectedTable(t)
        ...
        <TabsContent value="map-food" className="mt-0">
          <div className="rounded-xl bg-white shadow-md p-2 sm:p-4 mb-4">
            <p className="text-sm text-gray-600 mb-2">
              ...
              (จ่ายได้เฉพาะโต๊ะที่<strong>เข้างานแล้ว</strong>)
            </p>
            <div className="flex flex-wrap gap-2 text-xs">
              <span className="inline-flex items-center gap-1">
                <span className="w-3 h-3 rounded-full bg-yellow-500" /> รออนุมัติ
